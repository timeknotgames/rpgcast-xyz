...
export function createTelnetServer(gameLoop: GameLoop, port: number = 4000): net.Server {
  const server = net.createServer((socket) => {
    let session: Session | null = null;
    let inputBuffer = '';
    let state: 'auth_prompt' | 'login_user' | 'login_pass' | 'register_user' | 'register_pass' | 'name' | 'system' | 'campaign' | 'char_create' | 'char_select' | 'playing' = 'auth_prompt';
    let authUser: MudUser | null = null;
    ...
    socket.on('data', async (data) => {
      ...
      while (inputBuffer.includes('\n') || inputBuffer.includes('\r')) {
        const lineEnd = Math.min(
          inputBuffer.includes('\r') ? inputBuffer.indexOf('\r') : Infinity,
        ...
        switch (state) {
          ...
          case 'login_user': {
            ...
            if (!pendingUsername) { send(`${ANSI.red}Enter a username.${ANSI.reset}`); break; }
          ...
          case 'login_pass': {
            ...
            if (result.success && result.user) {
              ...
              if (characters.length > 0) {
                ...
                socket.write(PROMPT);
                (socket as any).__characters = characters;
                state = 'char_select';
              } else {
          ...
          case 'register_user': {
            ...
            if (pendingUsername.length < 2) { send(`${ANSI.red}Username must be at least 2 characters.${ANSI.reset}`); break; }
          ...
          case 'char_select': {
            const characters = (socket as any).__characters || [];
            ...
            const sys = char.activeSystem as GameSystem;
          ...
          case 'name': {
            ...
            (socket as any).__pendingName = name;
            state = 'system';
            break;
          }
          ...
          case 'system': {
            ...
            (socket as any).__pendingSystem = system;
          ...
          case 'campaign': {
            const campaignMap: Record<string, { room: string; label: string }> = {
            ...
            (socket as any).__startRoom = campaign.room;
            (socket as any).__campaignLabel = campaign.label;

            const system = (socket as any).__pendingSystem as GameSystem;
            ...
            if (system === 'dice-godz') {
              ...
              const name = (socket as any).__pendingName || 'Adventurer';
          ...
          case 'char_create': {
            ...
            const name = (socket as any).__pendingName || 'Adventurer';
            const system = ((socket as any).__pendingSystem || 'dice-godz') as GameSystem;
            const startRoom = (socket as any).__startRoom || 'R01';
            const campaignLabel = (socket as any).__campaignLabel || 'the Crying Depths';
