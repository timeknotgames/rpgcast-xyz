...
import type {
  MM3eCharacterSheet, MM3eAbility, MM3ePower,
  MM3eCondition, CombatResult, DiceRoll,
} from '../engine/types.js';
import { rolld20, rollCheck, narrateRoll } from '../engine/dice.js';

// ── CONSTANTS ──

// Condition ladder (graduated conditions)
export const DAMAGE_CONDITIONS = ['', 'Dazed', 'Staggered', 'Incapacitated'] as const;
...
export function mm3eCheck(
  rank: number,
  dc: number,
  label?: string
): { roll: DiceRoll; total: number; success: boolean; margin: number; narrative: string } {
  const check = rollCheck(rank, dc);
  const prefix = label ? `${label}: ` : '';

  let narrative = `${prefix}d20(${check.roll.roll}) + ${rank} = ${check.total} vs DC ${dc}`;
  if (check.nat20) narrative += ' — NAT 20!';
  else if (check.nat1) narrative += ' — NAT 1!';
...
export function mm3eAttack(
  ...
  if (nat20) narrative += ' — NAT 20!';
  else if (nat1) narrative += ' — NAT 1! Auto-miss!';
  narrative += (check.success || nat20) && !nat1 ? ' — Hit!' : ' — Miss!';
  if (criticalHit) narrative += ' 💥 CRITICAL!';
...
// ── RESISTANCE CHECKS (TOUGHNESS, FORT, WILL) ──
...
export interface ResistanceResult {
  ...
  dc: number;
  margin: number;
  degrees: number;        // Degrees of failure (0 = success, 1-4 = failure degrees)
 ...
 */
export function mm3eResistanceCheck(
  resistanceBonus: number,
  effectRank: number,
  resistanceType: 'Toughness' | 'Fortitude' | 'Will' | 'Dodge',
  bruisePenalty: number = 0
): ResistanceResult {
  ...
  if (!check.success) {
    ...
    if (failMargin >= 16) degrees = 4;
    else if (failMargin >= 11) degrees = 3;
    else if (failMargin >= 6) degrees = 2;
    else degrees = 1;
  }

  // Apply conditions based on resistance type and degrees
  if (resistanceType === 'Toughness') {
    if (degrees >= 1) conditions.push('Bruise (-1 Toughness)');
    if (degrees >= 2) conditions.push('Dazed (1 round)');
    if (degrees >= 3) conditions.push('Staggered');
    if (degrees >= 4) conditions.push('Incapacitated');
  } else if (resistanceType === 'Fortitude') {
    if (degrees >= 1) conditions.push('1st degree effect');
    if (degrees >= 2) conditions.push('2nd degree effect');
    if (degrees >= 3) conditions.push('3rd degree effect');
  ...
  if (bruisePenalty > 0) narrative += ` (includes -${bruisePenalty} bruise penalty)`;
...
export function usePower(
  sheet: MM3eCharacterSheet,
  powerName: string
): { success: boolean; power?: MM3ePower; narrative: string } {
  const power = sheet.powers.find(p =>
...
export function mm3eInitiative(agilityRank: number, improvedInit: boolean = false): {
  total: number;
  roll: DiceRoll;
  narrative: string;
} {
...
export function isIncapacitated(sheet: MM3eCharacterSheet): boolean {
  return sheet.conditions.some(c =>
    c.name === 'Incapacitated' || c.name === 'Asleep' || c.name === 'Paralyzed'
  );
}
...
export function checkDefensePLCap(
  powerLevel: number,
  dodgeOrParry: number,
  toughnessOrFortOrWill: number
): { valid: boolean; message: string } {
...
export function renderMM3eSheet(sheet: MM3eCharacterSheet): string {
  const lines: string[] = [];
  ...
  const abilities: MM3eAbility[] = ['STR', 'STA', 'AGL', 'DEX', 'FGT', 'INT', 'AWE', 'PRE'];
