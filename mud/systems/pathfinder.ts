...
import type {
  PathfinderCharacterSheet, PFAbility, PFAbilityScores,
  PFSave, PFAlignment, PFSpell, PFClassFeature, CombatResult, DiceRoll,
} from '../engine/types.js';
import { rollCheck, rolld20, rollMultiple, rollDie, narrateRoll } from '../engine/dice.js';
...
export function calculateAbilityMods(scores: PFAbilityScores): PFAbilityScores {
  return {
...
export function generateAbilityScores(): PFAbilityScores {
  const abilities: PFAbility[] = ['STR', 'DEX', 'CON', 'INT', 'WIS', 'CHA'];
  const scores = {} as PFAbilityScores;
...
export function pfAttack(
  ...
  damageBonus: number,
  critRange: number = 20,   // Threat on this or higher (20 = only nat 20)
  ...
  else if (nat1) narrative += ' — NAT 1! Automatic miss!';
...
export function pfSavingThrow(
  saveBonus: number,
  dc: number,
  saveType: PFSave
): { success: boolean; total: number; roll: DiceRoll; narrative: string } {
  ...
  if (check.nat20) narrative += ' — NAT 20! Auto-success!';
  else if (check.nat1) narrative += ' — NAT 1! Auto-fail!';
  else if (check.success) narrative += ' — Success!';
  else narrative += ' — Failed!';

  return { success: check.success, total: check.total, roll: check.roll, narrative };
}

/** Make a PF1e skill check */
export function pfSkillCheck(
  skillBonus: number,
  dc: number,
  skillName: string,
  take10?: boolean,
...
/** Roll initiative */
export function pfInitiative(dexMod: number, otherMods: number = 0): {
  total: number;
  roll: DiceRoll;
  narrative: string;
} {
...
export function calculateCMB(bab: number, strMod: number, sizeMod: number = 0): number {
  return bab + strMod + sizeMod;
}

/** Calculate Combat Maneuver Defense */
export function calculateCMD(bab: number, strMod: number, dexMod: number, sizeMod: number = 0): number {
...
/** Calculate spell DC */
export function spellDC(spellLevel: number, castingMod: number): number {
  return 10 + spellLevel + castingMod;
}

/** Use a spell (decrement prepared count) */
export function castSpell(
  sheet: PathfinderCharacterSheet,
  spellName: string
): { success: boolean; spell?: PFSpell; narrative: string } {
...
export function useClassFeature(
  sheet: PathfinderCharacterSheet,
  featureName: string
): { success: boolean; feature?: PFClassFeature; narrative: string } {
...
export function renderPathfinderSheet(sheet: PathfinderCharacterSheet): string {
  const lines: string[] = [];
