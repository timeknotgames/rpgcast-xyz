...
import type {
  GameSystem, PlayerState, CombatResult, CombatAction, Element, CoreElement,
  DiceRoll, CombatState, RoomNPC,
} from '../engine/types.js';
import { rollDie } from '../engine/dice.js';
import { getNPCProfile, type NPCCombatProfile } from '../data/npcs.js';
import {
  attainmentCheck, resolve360Combat, renderDiceGodzSheet,
} from './dice-godz.js';
import {
  pfAttack, pfSavingThrow, pfSkillCheck, pfInitiative,
  abilityMod, renderPathfinderSheet,
} from './pathfinder.js';
import {
  mm3eAttack, mm3eResistanceCheck, mm3eInitiative, mm3eCheck,
  renderMM3eSheet, addCondition,
} from './mm3e.js';
...
export function performCheck(
  ...
  switch (player.activeSystem) {
    ...
    case 'pathfinder': {
      if (!player.pathfinderSheet) return { success: false, narrative: '❌ No Pathfinder sheet loaded.' };
    ...
    case 'mm3e': {
      if (!player.mm3eSheet) return { success: false, narrative: '❌ No M&M 3e sheet loaded.' };
      ...
      const rank = player.mm3eSheet.abilities[abilityKey as keyof typeof player.mm3eSheet.abilities] || 0;
...
export function performAttack(
  ...
  if (!profile) return { hit: false, damage: 0, narrative: `❌ No combat stats found for ${npc.name}.` };

  switch (player.activeSystem) {
    case 'dice-godz': {
      if (!player.diceGodzSheet) return { hit: false, damage: 0, narrative: '❌ No Dice Godz sheet.' };
      const attackElement = (action.element || 'Air') as CoreElement;
      const defenderElement = profile.diceGodz.primaryElement;
      const result = resolve360Combat(
        attackElement,
        defenderElement,
        player.diceGodzSheet.currentAngle,
      );
      return {
        hit: result.success,
    ...
    case 'pathfinder': {
      if (!player.pathfinderSheet) return { hit: false, damage: 0, narrative: '❌ No PF1e sheet.' };
      const sheet = player.pathfinderSheet;
      const strMod = abilityMod(sheet.abilities.STR);
      const result = pfAttack(
        sheet.bab,
        strMod,
        0,
        profile.pathfinder.ac,
        { count: 1, sides: 8 }, // Default weapon damage
        strMod,
      );
      return {
        hit: result.hit,
    ...
    case 'mm3e': {
      if (!player.mm3eSheet) return { hit: false, damage: 0, narrative: '❌ No M&M 3e sheet.' };
      ...
      if (result.hit) {
        ...
        const resist = mm3eResistanceCheck(
          profile.mm3e.defenses.toughness,
          effectRank,
          'Toughness'
        );
...
export function rollInitiative(player: PlayerState): { total: number; narrative: string } {
  switch (player.activeSystem) {
    case 'dice-godz': {
    ...
    case 'pathfinder': {
      if (!player.pathfinderSheet) return { total: 0, narrative: '❌ No PF1e sheet.' };
    ...
    case 'mm3e': {
      if (!player.mm3eSheet) return { total: 0, narrative: '❌ No M&M 3e sheet.' };
...
export function renderCharacterSheet(player: PlayerState): string {
  const header = `🎮 Active System: ${player.activeSystem.toUpperCase()}\n${'═'.repeat(48)}\n`;

  switch (player.activeSystem) {
    case 'dice-godz':
      if (!player.diceGodzSheet) return header + '❌ No Dice Godz character created. Use "roll gwe" to begin.';
      return header + renderDiceGodzSheet(player.diceGodzSheet);

    case 'pathfinder':
      if (!player.pathfinderSheet) return header + '❌ No Pathfinder character loaded.';
      return header + renderPathfinderSheet(player.pathfinderSheet);

    case 'mm3e':
      if (!player.mm3eSheet) return header + '❌ No M&M 3e character loaded.';
...
export function switchSystem(player: PlayerState, system: string): string {
  const valid: GameSystem[] = ['dice-godz', 'pathfinder', 'mm3e'];
  const normalized = system.toLowerCase().replace(/\s+/g, '-') as GameSystem;
