...
export async function registerUser(
  username: string,
  password: string,
  authSource: 'local' | 'itch.io' | 'solana' = 'local',
): Promise<AuthResult> {
  if (!db) return { success: false, message: 'Persistence not available. Play as guest.' };
  ...
  if (existing.length > 0) return { success: false, message: `Username "${cleanName}" is already taken.` };
...
export async function loginUser(username: string, password: string): Promise<AuthResult> {
  ...
  if (users.length === 0) return { success: false, message: 'Unknown username. Type "register" to create an account.' };

  const user = users[0];
  if (!user.password) return { success: false, message: 'This account uses OAuth/wallet login. Try itch.io or wallet auth.' };
...
// ── WALLET LOGIN ──
export async function walletLogin(walletAddress: string, signature: number[], message: string): Promise<AuthResult> {
...
export async function linkWallet(userId: number, walletAddress: string): Promise<AuthResult> {
  if (!db) return { success: false, message: 'Persistence not available.' };
...
// ── SESSION VALIDATION ──
export async function validateSession(token: string): Promise<MudUser | null> {
  if (!db) return null;
...
export async function loadCharacters(userId: number): Promise<CharacterLoadResult> {
  if (!db) return { success: false, characters: [] };
  const chars = await db.select().from(mudCharacters).where(eq(mudCharacters.userId, userId));
  return { success: true, characters: chars };
}

export async function createCharacter(userId: number, name: string, system: string, emoji: string = '🧙'): Promise<MudCharacter | null> {
  if (!db) return null;
...
export async function saveCharacter(characterId: number, data: Partial<{
  ...
  await db.update(mudCharacters).set({
    ...
    updatedAt: new Date(),
    lastPlayed: new Date(),
  } as any).where(eq(mudCharacters.id, characterId));
}

export async function setCharacterPublic(characterId: number, isPublic: boolean): Promise<void> {
  if (!db) return;
  await db.update(mudCharacters).set({ isPublic }).where(eq(mudCharacters.id, characterId));
}

export async function getPublicCharacters(): Promise<MudCharacter[]> {
  if (!db) return [];
...
export async function isUserGM(userId: number): Promise<boolean> {
  if (!db) return false;
  const users = await db.select().from(mudUsers).where(eq(mudUsers.id, userId));
