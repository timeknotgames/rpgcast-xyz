...
export class World {
    ...
    movePlayer(playerId, direction) {
        ...
        return {
            success: true,
            message: `${ENV_EMOJI[exit.direction] || '🚶'} You move ${exit.direction}.\n\n${desc}\n\n${roomView}${eventText}`,
            newRoom,
        };
    }
    // ── ROOM RENDERING ──
    renderRoom(room, player) {
        const occupants = [];
        // Add NPCs
    ...
    dropItem(playerId, itemName) {
        ...
        if (!room)
            return '❌ Room not found.';
        const invIndex = player.inventory.findIndex(i => i.name.toLowerCase().includes(itemName.toLowerCase()));
        if (invIndex === -1)
    ...
    talkToNPC(playerId, npcName) {
        ...
        if (!room)
            return '❌ Room not found.';
        const npc = room.npcs.find(n => n.name.toLowerCase().includes(npcName.toLowerCase()));
        if (!npc)
    ...
    renderMinimap(playerId) {
        ...
        for (const exit of room.exits) {
            ...
            lines.push(`   ${ENV_EMOJI[exit.direction] || '🚪'} ${exit.direction} → ${targetRoom?.name || '???'} [${visited}]${locked}`);
        }
        lines.push('');
