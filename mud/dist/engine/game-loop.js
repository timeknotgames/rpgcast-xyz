...
import { rollDie, rollCheck } from './dice.js';
import { attainmentCheck, createGodzWorldEgg, resolve360Combat, renderDiceGodzSheet, } from '../systems/dice-godz.js';
import { pfAttack, renderPathfinderSheet } from '../systems/pathfinder.js';
import { mm3eAttack, renderMM3eSheet } from '../systems/mm3e.js';
import { getAllSessions, broadcastToRoom, broadcastGlobal, saveSessionCharacter } from '../server/session.js';
...
export class GameLoop {
    ...
    processInput(playerId, rawInput) {
        const player = this.world.getPlayer(playerId);
        if (!player)
            return '❌ Session not found. Try reconnecting.';
        ...
        if (!parsed) {
            ...
            if (room) {
                ...
                if (matchItem)
                    return `💡 To pick up ${matchItem.emoji} ${matchItem.name}, type: take ${matchItem.name.toLowerCase().split(' ')[0]}`;
                const matchNpc = room.npcs?.find(n => n.name.toLowerCase().includes(word) || word.includes(n.name.toLowerCase()));
                if (matchNpc)
                    return `💡 To interact with ${matchNpc.emoji} ${matchNpc.name}, try: talk ${matchNpc.name.toLowerCase().split(' ')[0]}`;
            }
    ...
    handleSearch(player) {
        ...
        switch (player.activeSystem) {
            ...
            case 'pathfinder': {
                const result = rollCheck((player.pathfinderSheet?.skills['Perception'] || 0), 15);
            ...
            case 'mm3e': {
                const result = rollCheck((player.mm3eSheet?.abilities.AWE || 0), 15);
    ...
    // ── ITEM USE ──
    handleUse(player, itemName) {
        if (!itemName)
            return '❓ Use what? (use <item name>)';
    ...
    // ── COMBAT ──
    handleAttack(player, args) {
        const room = this.world.getRoom(player.currentRoomId);
        if (!room)
            return '❌ Room not found.';
        const targetName = args.join(' ');
        if (!targetName)
            return '❓ Attack whom? (attack <target>)';
    ...
    handleFlee(player) {
        ...
        if (validExits.length === 0)
            return '❌ All exits are locked! You cannot flee!';
    ...
    handleCheck(player, args) {
        ...
        if (elementArg && CORE_ELEMENTS.includes(elementArg)) {
            return this.doRoomCheck(player, room, elementArg);
        }
        // Show available checks
        if (room.checks.length === 0)
            return '🔍 There are no special checks available here.';
    ...
    doRoomCheck(player, room, element) {
        const check = room.checks.find(c => c.element === element);
        if (!check)
            return `❌ No ${element} check available here.`;
        switch (player.activeSystem) {
            case 'dice-godz': {
            ...
            case 'mm3e': {
                ...
                const rank = player.mm3eSheet?.abilities?.[ability] || 0;
                const dc = 10 + Math.round(check.attainment / 5);
    ...
    handleRoll(player, args) {
        ...
        if (what && what.match(/^d?\d+$/)) {
            ...
            if (max < 2 || max > 100)
                return '❌ Roll a die between d2 and d100.';
        ...
        if (elementArg && CORE_ELEMENTS.includes(elementArg)) {
            const die = ELEMENT_DICE[elementArg];
            const result = rollDie(die);
            const emoji = ELEMENT_EMOJI[elementArg];
    ...
    handleCast(player, args) {
        ...
        if (player.activeSystem === 'pathfinder' && player.pathfinderSheet) {
            const spell = player.pathfinderSheet.spells.find(s => s.name.toLowerCase().includes(spellName.toLowerCase()));
            if (!spell)
                return `❌ You don't know "${spellName}".`;
            if (spell.prepared <= 0)
                return `❌ No more ${spell.name} prepared.`;
            spell.prepared--;
            return `✨ You cast ${spell.name}! (${spell.prepared} uses remaining)\n${spell.description}`;
        }
        if (player.activeSystem === 'mm3e' && player.mm3eSheet) {
            const power = player.mm3eSheet.powers.find(p => p.name.toLowerCase().includes(spellName.toLowerCase()));
            if (!power)
                return `❌ You don't have the power "${spellName}".`;
    ...
    handleMusic(player, args) {
        ...
        for (const event of musicEvents) {
            if (event.oneShot && player.flags[`event_${event.id}`])
                continue;
            response += `\n${event.description}`;
            if (event.oneShot)
        ...
        switch (player.activeSystem) {
            case 'dice-godz': {
                if (!player.diceGodzSheet)
                    return '❌ No Dice Godz character yet. Type "roll gwe" to create one.';
                return renderDiceGodzSheet(player.diceGodzSheet);
            }
            case 'pathfinder': {
                if (!player.pathfinderSheet)
                    return '❌ No Pathfinder character sheet loaded.';
                return renderPathfinderSheet(player.pathfinderSheet);
            }
            case 'mm3e': {
                if (!player.mm3eSheet)
                    return '❌ No M&M 3e character sheet loaded.';
                return renderMM3eSheet(player.mm3eSheet);
            }
        }
    }
    // ── SYSTEM SWITCHING ──
    handleSystemSwitch(player, system) {
        if (!system) {
        ...
        if (!newSystem)
            return '❌ Unknown system. Choose: dice-godz, pathfinder, or mm3e';
    ...
    handleSay(player, message) {
        if (!message)
            return '❓ Say what?';
        broadcastToRoom(player.currentRoomId, `💬 ${player.emoji} ${player.name} says: "${message}"`, player.id);
    ...
    handleWhisper(player, args) {
        if (args.length < 2)
            return '❓ whisper <name> <message>';
        const targetName = args[0];
        const message = args.slice(1).join(' ');
        const sessions = getAllSessions();
        const target = sessions.find(s => s.playerState.name.toLowerCase().includes(targetName.toLowerCase()));
        if (!target)
    ...
    // ── GM COMMANDS ──
    handleGM(playerId, args) {
        const session = getAllSessions().find(s => s.playerState.id === playerId);
        if (!session?.dbUser)
            return '❌ You must be logged in to use GM commands.';
