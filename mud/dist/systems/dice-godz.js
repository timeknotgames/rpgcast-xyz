...
import { rollDie, rollElement, formatAttainment } from '../engine/dice.js';
...
export function rollGWESet(setNumber, remainingAngle, totalSets, specializedElements) {
    let diceToRoll;
...
export function calculateElementalComposition(sets) {
    const percentages = {};
    const eggPercentages = {};
    let aaaTotal = 0;
...
export function calculateKarma(sets) {
    let totalRolled = 0;
    let totalPossible = 0;
    for (const set of sets) {
        for (const element of CORE_ELEMENTS) {
...
export function calculateOP(karma) {
...
export function createGodzWorldEgg() {
    const sets = [];
    let totalAngle = 0;
    let setNumber = 0;
...
export function distributeOpulences(sheet, distribution) {
...
export function attainmentCheck(element, requiredPercent, bonusPercent = 0) {
...
export function angleModifier(distance) {
    if (distance <= 22.5)
        return 0.25; // Primary advantage
    if (distance <= 45)
        return 0.15; // Secondary advantage
    if (distance <= 90)
        return 0; // Neutral
    if (distance <= 135)
        return -0.15; // Disadvantage
    return -0.25; // Severe disadvantage
}
/** Resolve 360° combat between attacker and defender */
export function resolve360Combat(attackerElement, defenderElement, attackAngle, attackerBonus = 0, defenderBonus = 0) {
    ...
    const attackerScore = Math.min(1, Math.max(0, attackerRoll.attainment + modifier + (attackerBonus / 100)));
    const defenderScore = Math.min(1, Math.max(0, defenderRoll.attainment + (modifier < 0 ? Math.abs(modifier) * 0.5 : 0) + (defenderBonus / 100)));
...
export function renderDiceGodzSheet(sheet) {
    const lines = [];
    lines.push('╔═══════════════════════════════════════════════╗');
...
//# sourceMappingURL=dice-godz.js.map