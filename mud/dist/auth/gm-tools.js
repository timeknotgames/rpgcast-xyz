...
import { VIDYAS, renderKalaList, getKala, getKalaByName, getKalasByElement, kalaCheck, renderKala } from '../data/kalas.js';
// ── CREATE CUSTOM ITEM ──
// Usage: /createitem <id> <emoji> <name> | <description> [| pickupable | element:Fire | attainment:50]
export async function createCustomItem(userId, itemId, name, emoji, description, options) {
    if (!db)
        return { success: false, message: 'Persistence not available.' };
    if (!(await isUserGM(userId)))
        return { success: false, message: 'GM privileges required.' };
    try {
        ...
        if (err.code === '23505')
            return { success: false, message: `Item ID "${itemId}" already exists.` };
        return { success: false, message: `Error creating item: ${err.message}` };
    }
}
// ── CREATE CUSTOM CLASS ──
export async function createCustomClass(userId, classId, name, gameSystem, description, baseStats, abilities = []) {
    if (!db)
        return { success: false, message: 'Persistence not available.' };
    if (!(await isUserGM(userId)))
        return { success: false, message: 'GM privileges required.' };
    try {
        ...
        if (err.code === '23505')
            return { success: false, message: `Class ID "${classId}" already exists.` };
        return { success: false, message: `Error creating class: ${err.message}` };
    }
}
// ── PROMOTE USER TO GM ──
export async function promoteToGM(adminId, targetUsername) {
    if (!db)
        return { success: false, message: 'Persistence not available.' };
...
export async function getLeaderboard(limit = 10) {
    if (!db)
        return 'Leaderboard unavailable (no database).';
    const entries = await db.select().from(mudLeaderboards)
        .orderBy(mudLeaderboards.roomsExplored)
        .limit(limit);
    if (entries.length === 0)
        return 'No leaderboard entries yet. Start exploring!';
...
export async function parseGMCommand(userId, input) {
    ...
    switch (subcommand) {
        ...
        case 'promote': {
            ...
            if (!targetName)
                return 'Usage: /gm promote <username>';
        ...
        case 'items': {
            ...
            if (items.length === 0)
                return 'No custom items created yet.';
        ...
        case 'kala': {
            ...
            if (isNaN(id) || id < 1 || id > 64)
                return 'Usage: /gm kala <1-64>';
            const kala = getKala(id);
            if (!kala)
                return 'Kala not found.';
        ...
        case 'createspecies': {
            ...
            const speciesResult = await createCustomClass(userId, speciesId, speciesName, 'species', description, { petal, kalaAffinity: kalaNum, kalaName: kala?.english, emoji, isBoundary: false }, [] // abilities filled via web admin or future commands
            );
        ...
        case 'species': {
            ...
            if (species.length === 0)
                return 'No custom species created yet.';
...
//# sourceMappingURL=gm-tools.js.map