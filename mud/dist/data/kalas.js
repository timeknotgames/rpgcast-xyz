...
export function createCustomSkill(slotId, name, description, abilities, element, affinity = 'standard') {
    return {
        slotId,
        name,
        description,
...
export function kalaCheck(skillValue, difficulty = 0) {
...
// ── LOOKUP HELPERS ──
export function getKala(id) {
    return KALAS.find(k => k.id === id);
}
export function getKalaByName(name) {
    const lower = name.toLowerCase();
    return KALAS.find(k => k.english.toLowerCase().includes(lower) ||
        k.sanskrit.toLowerCase().includes(lower));
}
export function getVidya(id) {
    return VIDYAS.find(v => v.id === id);
}
export function getKalasByElement(element) {
    return KALAS.filter(k => k.element === element);
}
export function getKalasByCategory(category) {
    return KALAS.filter(k => k.category === category);
}
/** Render a Kala for display in the MUD */
export function renderKala(kala) {
...
export function renderKalaList() {
    const categories = ['performing', 'social', 'intellectual', 'technical', 'mystical', 'mastery'];
    const lines = ['═══ THE 64 KALAS (Sacred Arts) ═══\n'];
